...
class SilencedJobsController extends Controller
{
    ...
    public function index(Request $request)
    {
        ...
        $total = $request->query('tag')
            ? $this->tags->count('silenced_jobs:'.$request->query('tag'))
            : $this->jobs->countSilenced();
    ...
    /**
     * Paginate the silenced jobs for the request.
     *
     * @param  \Illuminate\Http\Request  $request
    ...
    /**
     * Paginate the silenced jobs for the request and tag.
     *
     * @param  \Illuminate\Http\Request  $request
     * @param  string  $tag
     * @return \Illuminate\Support\Collection
    ...
    protected function paginateByTag(Request $request, $tag)
    {
        $jobIds = $this->tags->paginate(
            'silenced_jobs:'.$tag, ($request->query('starting_at') ?: -1) + 1, 50
        );
