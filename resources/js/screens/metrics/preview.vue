        ...
        methods: {
            ...
            loadMetric() {
                ...
                this.$http.get(Horizon.basePath + '/api/metrics/' + this.$route.params.type + '/' + encodeURIComponent(this.$route.params.slug))
                    .then(response => {
                        ...
                        this.metric.memoryUsageChart = this.buildChartData(data, 'memory', 'MB');
            ...
            prepareData(data) {
                return Object.values(this.groupBy(data.map(value => ({
                    ...
                    memory: parseFloat(sum.memory || 0) + parseFloat(value.memory || 0),
                    time: value.time
                })))
            },
...
<template>
    <div>
        ...
        <div class="card overflow-hidden mt-4" v-if="ready && rawData.some(item => item.memory)">
            <div class="card-header d-flex align-items-center justify-content-between">
                <h2 class="h6 m-0">Memory Usage (MB) - {{$route.params.slug}}</h2>
            </div>
            ...
            <div class="card-body card-bg-secondary" v-if="ready">
                ...
                <line-chart v-if="ready && rawData.length" :data="metric.memoryUsageChart"/>
            </div>
        </div>
    </div>
</template>
