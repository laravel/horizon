<script type="text/ecmascript-6">
    ...
    export default {
        ...
        methods: {
            ...
            /**
             * Build a markdown summary of the failed job.
             *
             * @returns {string}
             */
            markdownSummary() {
                let payload = this.job.payload || {};
                let tags = payload.tags && payload.tags.length ? payload.tags.join(', ') : '';
                let data = this.prettyPrintJob(payload.data);
                let context = this.prettyPrintJob(this.job.context);

                let lines = [
                    `# Failed Job: ${this.job.name}`,
                    '',
                    `- **ID**: ${this.job.id}`,
                ...
                if (payload.retry_of) {
                    lines.push(`- **Retry of ID**: ${payload.retry_of}`);
                }
                ...
                lines.push(`- **Failed**: ${this.readableTimestamp(this.job.failed_at)}`);

                lines.push('', '## Exception', '', '```', this.job.exception, '```');
                lines.push('', '## Exception Context', '', '```json', this.stringify(context), '```');
                lines.push('', '## Data', '', '```json', this.stringify(data), '```');

                if (this.job.retried_by.length) {
                    lines.push('', '## Recent Retries', '', '| Status | ID | Retry Time |', '| --- | --- | --- |');

                    this.job.retried_by.forEach(retry => {
                        lines.push(`| ${this.upperFirst(retry.status)} | ${retry.id} | ${this.readableTimestamp(retry.retried_at)} |`);
                    });
                }
            ...
            /**
             * Stringify the given value as pretty printed JSON.
             *
             * @param value
             * @returns {string}
             */
            ...
            /**
             * Copy the markdown summary of the failed job to the clipboard.
             */
            copySummary() {
                this.writeToClipboard(this.markdownSummary());
        ...
        <div class="card overflow-hidden">
            <div class="card-header d-flex align-items-center justify-content-between">
                ...
                <div class="d-flex align-items-center">
                    <button class="btn btn-secondary me-2" v-if="ready" v-on:click.prevent="copySummary">
