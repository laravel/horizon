...
export default {
  props: ["assetsAreCurrent", "isDownForMaintenance", "appName"],
  data() {
    return {
  ...
  <div>
    ...
    <div class="container mb-5">
      <div class="d-flex align-items-center py-4 header">
        ...
        <h4 class="mb-0 ml-2"><strong>Laravel</strong> Horizon{{ appName }}</h4>

        <button
          class="btn btn-outline-primary ml-auto"
          :class="{ active: this.$root.autoLoadsNewEntries }"
      ...
      <div class="row mt-4">
        ...
        <div class="col-10">
          <div v-if="!assetsAreCurrent" class="alert alert-warning">
            The published Horizon assets are not up-to-date with the installed
            version. To update, run:<br /><code
              >php artisan horizon:publish</code
            >
          </div>

          <div v-if="isDownForMaintenance" class="alert alert-warning">
            This application is in "maintenance mode". Queued jobs may not be
            processed unless your worker is using the "force" flag.
          </div>
