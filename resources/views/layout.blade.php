...
<div id="horizon" v-cloak>
    ...
    <div class="container mb-5">
        <div class="d-flex align-items-center py-4 header">
            ...
            <div class="dropdown" style="margin-left: 5px" v-on:change.prevent="changeLanguage">
                <button class="btn btn-muted dropdown-toggle ml-auto" type="button" id="dropdownMenuButton" data-toggle="dropdown" aria-haspopup="true" aria-expanded="false">
                    @{{ language[selectedLanguage] }}
                </button>
                <div class="dropdown-menu dropdown-menu-right" aria-labelledby="dropdownMenuButton">
                    <a class="dropdown-item" v-on:click.prevent="changeLanguage(key)" v-for="(value, key) in language" :key="key">@{{ value }}</a>
                </div>
            </div>
        </div>
            ...
            <div class="col-10">
                @if (! $assetsAreCurrent)
                    <div class="alert alert-warning">
                        @{{ $t("The published Horizon assets are not up-to-date with the installed version. To update, run:") }}<br/><code>php artisan horizon:publish</code>
                    </div>
                @endif

                @if ($isDownForMaintenance)
                    <div class="alert alert-warning">
                        @{{ $t("This application is in \"maintenance mode\",. Queued jobs may not be processed unless your worker is using the \"force\" flag.") }}
                    </div>
                @endif
